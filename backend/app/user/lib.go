...
func New(username, password, frontendSeed, backendSeed string, token *oauth2.Token) User {
	session := hash.SHA256(frontendSeed, backendSeed).Hex()
	salt := random.String(32)
	if token == nil {
		token = &oauth2.Token{}
	}
	// same session value is not a problem as long as we don't do db.Where("session = ?", session).Find(&u) so I don't care
	u := User{
